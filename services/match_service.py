...
async def recalculate_matches_for_user(r, trigger_username: str):
    ...
    log.info(f"[RECALC] ◀ concluído para '{trigger_username}' em {elapsed_total:.3f}s — "
...
async def _close_match(r, chain: list) -> bool:
    ...
    log.debug(f"[CLOSE] [⏱ {time.perf_counter()-t0:.3f}s] total _close_match para {chain_usernames}")
    return True
